...
**Audits (stratégie, ordre d’exécution, rapports)** : voir `docs/audit/governance/docs/STANDARDS/README.md` et `scripts/audit/governance/docs/STANDARDS/README.md`.
...
2. **gate:route-catalog** — `runtime/configs/ssot/ROUTE_CATALOG.json` existe, JSON valide, `routes[]` non vide, `route_id` + `app_surface`  
3. **gate:route-drift** — `docs/ssot/ROUTE_DRIFT_REPORT.md` existe, « Routes extra » = 0 (régénérer : `npm run generate:route-drift`)  
4. **gate:tenant-matrix** — `runtime/configs/ssot/TENANT_FEATURE_MATRIX.json` existe, `templates` non vide, chaque plan a `enabled_pages[]` et `enabled_capabilities[]`  
5. **gate:design-tokens** — `runtime/configs/ssot/design.tokens.json` existe, JSON valide, au moins un de `base` / `presets` / `cssVarsMapping` (object)  
6. **gate:capability-status** — `runtime/configs/ssot/CAPABILITY_STATUS.json` existe, `capabilities[]` non vide, chaque entrée a `id`  
7. **gate:admin-components-registry** — `runtime/configs/ssot/ADMIN_COMPONENTS_REGISTRY.ts` existe, export `AdminComponentEntry` et `ADMIN_COMPONENTS_REGISTRY = []`  
...
| gate:route-catalog | `runtime/configs/ssot/ROUTE_CATALOG.json` |
| gate:route-drift | `docs/ssot/ROUTE_DRIFT_REPORT.md` — « Routes extra » = 0 (génération : `generate:route-drift`) |
| gate:tenant-matrix | `runtime/configs/ssot/TENANT_FEATURE_MATRIX.json` |
| gate:design-tokens | `runtime/configs/ssot/design.tokens.json` |
| gate:capability-status | `runtime/configs/ssot/CAPABILITY_STATUS.json` |
| gate:admin-components-registry | `runtime/configs/ssot/ADMIN_COMPONENTS_REGISTRY.ts` |
| gate:check-cross-imports | `app/src/pages/cp` ↔ `app/src/pages/app` |

Fichiers JSON et `design.tokens` : `iCONTROL/runtime/configs/ssot/`.
