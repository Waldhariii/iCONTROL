...
function checkLogContract() {
  ...
  // Targets: platform/api/*.ts + platform/api/*.mjs + platform/api/*.js (except wrapper is allowlisted)
  const candidates = [];
...
function checkWrapperPurity() {
  ...
  if (!content.includes("platform/api/dist/runtime-config-server.mjs")) {
    fail(
      "WRAPPER_PURITY",
      "Wrapper must delegate to platform/api/dist/runtime-config-server.mjs",
    );
  }
