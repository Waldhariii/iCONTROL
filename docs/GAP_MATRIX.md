...
|----------------|------|--------------------|-------|--------|
| Identity & Access (utilisateurs, invites, groups) | Partiel | `app/src/core/governance/rbac/`, `config/permissions/rbac.json`, localAuth, control-plane/storage | rbac, control-plane tests | Compléter ABAC, policies, scopes |
...
| SAFE_MODE global + par tenant + par module | Partiel | safeMode.ts, config/safe-mode/*.json | safe-mode-write.test | Brancher par tenant/module, config |
...
| Capacité cible | État | Fichiers / Preuves | Tests | Action |
|----------------|------|--------------------|-------|--------|
| Tokens centralisés (couleurs, spacing, typo) | Complet | config/ssot/design.tokens.json, core/theme | gate:design-tokens | — |
