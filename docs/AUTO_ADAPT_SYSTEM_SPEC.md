...
- **Recommandation (audit):** Option B ou hybride : générer dans `config/ssot/` ou `docs/ssot/` avec gate de non-régression (comme gate:route-drift).
...
- **Tokens centralisés :** source unique `config/ssot/design.tokens.json` ; génération CSS vars via `generate-design-tokens-css.mjs`.
