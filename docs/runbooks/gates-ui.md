...
- **gate:route-catalog** : `config/ssot/ROUTE_CATALOG.json` valide (structure, route_id, app_surface)
- **gate:route-drift** : `docs/ssot/ROUTE_DRIFT_REPORT.md` — « Routes extra » = 0. Régénérer avec `npm run generate:route-drift`.
- **gate:tenant-matrix** : `config/ssot/TENANT_FEATURE_MATRIX.json` valide (templates, enabled_pages, enabled_capabilities)
- **gate:design-tokens** : `config/ssot/design.tokens.json` valide (base / presets / cssVarsMapping)
- **gate:capability-status** : `config/ssot/CAPABILITY_STATUS.json` valide (capabilities[], id)
- **gate:admin-components-registry** : `config/ssot/ADMIN_COMPONENTS_REGISTRY.ts` valide (AdminComponentEntry, ADMIN_COMPONENTS_REGISTRY = [])
