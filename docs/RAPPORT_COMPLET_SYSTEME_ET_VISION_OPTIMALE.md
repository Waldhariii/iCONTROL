...
| **Config** | `config/` : brand/, branding/, permissions/rbac.json, safe-mode/*.json, ssot/ (ROUTE_CATALOG.json, TENANT_FEATURE_MATRIX.json, design.tokens.json, CAPABILITY_STATUS.json, ADMIN_COMPONENTS_REGISTRY.ts), module-registry.json. |
...
| **Route catalog** | `config/ssot/ROUTE_CATALOG.json` — routes avec route_id, path, app_surface (CP/CLIENT), page_module_id, permissions_required, tenant_visibility, status. |
...
| Élément | Détail |
|--------|--------|
| **SAFE_MODE** | `app/src/core/runtime/safeMode.ts` — isSafeMode() (window.__ICONTROL_SAFE_MODE__ ou localStorage icontrol.runtime.safeMode.v1). Bloque écritures (auditLog, entitlements, etc.). setSafeMode(on). Config : config/safe-mode/*.json. |
| **RBAC** | config/permissions/rbac.json (USER, ADMIN, SYSADMIN, DEVELOPER). app/src/core/governance/rbac/ (enforce, policy, ranks, types). app/src/core/runtime/rbac.ts (canAccessSettings, canAccessToolbox). |
...
| Élément | Détail |
|--------|--------|
| **Tokens** | config/ssot/design.tokens.json (base, cp, presets cp-dashboard-charcoal, app-foundation-slate). app/src/core/theme/ (themeTokens, loadPreset, applyThemeCssVars, themeManifest, presets/*.json). |
...
| **Module registry** | config/module-registry.json — core-system (core), dossiers, clients, inventory, documents, ocr, finance, billing, quotes, jobs, calendar, reports, contacts, payments, integrations-hub (complementary). |
...
| **Plan visuel** | docs/ssot/STYLE_INVENTORY, DESIGN_SYSTEM_SSOT. Tokens et arborescence UI dans config/ssot et core/theme. |
| **Routes** | Suffixes _cp (Control Plane) et _app (Client). Pas de route partagée entre les deux surfaces. |
| **Paths canoniques** | PATHS_CANONICAL.md, docs/ssot/PATHS_CANONICAL.md. |
