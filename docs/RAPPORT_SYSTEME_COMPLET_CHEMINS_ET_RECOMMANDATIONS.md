...
- **Racines de code :** `app/src`, `modules`, `platform-services`, `server` (cf. PATHS_CANONICAL.md).
...
| Catalogue routes (SSOT) | `config/ssot/ROUTE_CATALOG.json` |
...
| Fichier | Chemin |
|---------|--------|
| ROUTE_CATALOG | `config/ssot/ROUTE_CATALOG.json` |
| TENANT_FEATURE_MATRIX | `config/ssot/TENANT_FEATURE_MATRIX.json` |
| design.tokens | `config/ssot/design.tokens.json` |
| CAPABILITY_STATUS | `config/ssot/CAPABILITY_STATUS.json` |
| ADMIN_COMPONENTS_REGISTRY | `config/ssot/ADMIN_COMPONENTS_REGISTRY.ts` |
| README SSOT | `config/ssot/README.md` |

## 1.4 Config gouvernance et permissions

| Fichier | Chemin |
|---------|--------|
| RBAC | `config/permissions/rbac.json` |
| Safe-mode (exemple, schéma) | `config/safe-mode/safe-mode.example.json`, `config/safe-mode.schema.json`, `config/safe-mode.enforcement.example.json`, `config/safe-mode.enforcement.schema.json` |
| Brand | `config/brand/brand.default.json`, `config/brand.override.local.json`, `config/brand.schema.json` |
| Branding | `config/branding/branding.json` |
...
| README audit | `scripts/audit/README.md` |
...
| Doc | Chemin |
|-----|--------|
| PATHS_CANONICAL (racine) | `PATHS_CANONICAL.md` |
| PATHS_CANONICAL (ssot) | `docs/ssot/PATHS_CANONICAL.md` |
...
| WRITE_GATEWAY_CONTRACT | `docs/governance/WRITE_GATEWAY_CONTRACT.md` |
...
**Source :** `config/ssot/ROUTE_CATALOG.json`.  
...
## 3.3 Rôles RBAC (config/permissions/rbac.json)
...
- À chaque nouvelle page : ajouter l’entrée dans `config/ssot/ROUTE_CATALOG.json`, puis brancher le rendu dans `app/src/pages/cp/registry.ts` ou `app/src/pages/app/registry.ts` et dans `app/src/moduleLoader.ts` (ou remplacer par le registry généré par auto-discovery).  
...
**Design system**  
- Utiliser uniquement les tokens de `config/ssot/design.tokens.json` (couleurs, espacements, typo) ; pas de couleurs en dur (hex, rgb, rgba) hors `var(--*)`.  
...
**Documentation**  
- Tenir à jour PATHS_CANONICAL.md et config/ssot lorsqu’un nouveau chemin canonique est introduit.  
...
| Route #/docs (DOCS_OCR) | `app/src/router.ts` (getRouteId), `app/src/moduleLoader.ts`, `config/ssot/ROUTE_CATALOG.json`, puis page stub ou module documents. |
| Auto-discovery | Créer script dans `scripts/ssot/` ou `scripts/gates/` (scan `modules/**/manifest`), sortie vers `config/ssot/` ou artefact + gate drift. |
