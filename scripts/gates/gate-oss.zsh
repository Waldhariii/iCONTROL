...
BAD_TRACKED="$(git ls-files -z | tr '\0' '\n' | rg --pcre2 '(^|/)\.DS_Store$|^apps/control-plane/dist/|^dist/|^node_modules/|^\.vite/|^\.cache/' || true)"
...
BAD_STAGED="$(git diff --cached --name-status | rg -v '^D\t' | rg --pcre2 '(^|/)\.DS_Store$|^apps/control-plane/dist/|^dist/|^node_modules/|^\.vite/|^\.cache/' || true)"
