...
ALLOW_RE='^(apps/control-plane/src/platform/(storage|write-gateway)/|apps/control-plane/src/core/|apps/control-plane/src/dev/|apps/control-plane/src/__tests__/)'
# Block direct localStorage usage anywhere else (prod hygiene)
hits="$(rg -n --hidden --glob '!**/node_modules/**' -S -e '\blocalStorage\.(setItem|getItem|removeItem|clear)\b' apps/control-plane/src 2>/dev/null || true)"
