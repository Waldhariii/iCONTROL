...
out = pathlib.Path("docs/reports/AUDIT_route_like_imports.json")
...
cp_files = analyze("docs/reports/AUDIT_cp_pages_registry.txt")
app_files = analyze("docs/reports/AUDIT_app_pages_registry.txt")
...
p = pathlib.Path("docs/reports/AUDIT_route_strings.txt")
if not p.exists():
...
out = pathlib.Path("docs/reports/AUDIT_route_duplicates.txt")
...
python3 - <<'PY'
import re, pathlib
p = pathlib.Path("docs/reports/AUDIT_client_v2_ssot.txt")
if not p.exists():
...
## Résultats
- Imports runtime/router.ts (hors tests): docs/reports/AUDIT_runtime_router_imports.txt
- Route-like imports parallèles: docs/reports/AUDIT_route_like_imports.json
- Registries: docs/reports/AUDIT_cp_pages_registry.txt / docs/reports/AUDIT_app_pages_registry.txt
- Duplications routes: docs/reports/AUDIT_route_duplicates.txt
- CLIENT_V2 mentions: docs/reports/AUDIT_client_v2_mentions.txt
