#!/usr/bin/env bash
# ICONTROL_APP_ADMIN_LAUNCHER_V1
# Lance l'application ADMINISTRATION et ouvre le navigateur
...
# Log file pour debug
LOG_FILE="/tmp/icontrol-admin.log"
echo "=== iCONTROL Administration Launch $(date) ===" > "$LOG_FILE"
...
if [ ! -d "$ROOT" ]; then
    osascript -e "display dialog \"Le répertoire du projet iCONTROL est introuvable:\n\n$ROOT\n\nVérifiez que le projet est bien installé à cet emplacement.\" buttons {\"OK\"} default button \"OK\" with title \"iCONTROL Administration - Erreur\"" 2>>"$LOG_FILE"
    cat "$LOG_FILE"
    sleep 3
    exit 1
fi
...
cd "$ROOT" 2>>"$LOG_FILE" || {
    osascript -e "display dialog \"Impossible de changer vers le répertoire:\n\n$ROOT\" buttons {\"OK\"} default button \"OK\" with title \"iCONTROL Administration - Erreur\"" 2>>"$LOG_FILE"
    cat "$LOG_FILE"
    sleep 3
    exit 1
}
...
if [ -x "/opt/homebrew/bin/npm" ]; then
    ...
    cat "$LOG_FILE"
    osascript -e 'display dialog "npm n'\''est pas installé.\nVeuillez installer npm avec Node.js." buttons {"OK"} default button "OK" with title "iCONTROL Administration - Erreur"'
    sleep 3
    exit 1
fi
...
if curl -s "http://${HOST}:${PORT}/cp/" >/dev/null 2>&1; then
    echo "Serveur déjà actif, ouverture du navigateur" >> "$LOG_FILE"
    open "http://${HOST}:${PORT}/cp/#/login" 2>>"$LOG_FILE" || true
    sleep 1
    exit 0
fi

# Afficher notification
osascript -e 'display notification "Démarrage du serveur iCONTROL Administration..." with title "iCONTROL"' 2>>"$LOG_FILE" || true
...
if ! pgrep -f "runtime-config-server.js" >/dev/null 2>&1; then
    ...
    if [ $BUILD_EXIT -ne 0 ]; then
        ...
        cat "$LOG_FILE"
        osascript -e "display dialog \"Erreur lors du build (code $BUILD_EXIT).\n\nConsultez le log:\n$LOG_FILE\" buttons {\"OK\"} default button \"OK\" with title \"iCONTROL Administration - Erreur\"" 2>>"$LOG_FILE" || true
        sleep 5
        exit 1
    fi
    ...
    if [ $WAITED -ge $MAX_WAIT ]; then
        ...
        osascript -e "display dialog \"Le serveur n'a pas démarré après $MAX_WAIT secondes.\n\nConsultez les logs:\n$LOG_FILE\n\n/tmp/icontrol-server.log\" buttons {\"OK\"} default button \"OK\" with title \"iCONTROL Administration - Erreur\""
        cat "$LOG_FILE"
        sleep 5
