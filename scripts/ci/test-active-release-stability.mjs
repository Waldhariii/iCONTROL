import { mkdtempSync, readdirSync, statSync, readFileSync, writeFileSync, mkdirSync, symlinkSync, rmSync } from "fs";
import { tmpdir } from "os";
import { join } from "path";
import { execSync } from "child_process";

const root = process.cwd();
const temp = mkdtempSync(join(tmpdir(), "icontrol-active-"));
...
try {
  ...
  run("node scripts/ci/compile.mjs active-002 dev", temp);

  const activePath = join(temp, "runtime/manifests/platform_manifest.active-002.json");
  const before = readFileSync(activePath, "utf-8");

  const previewId = "cs-preview-002";
  ...
  run("node scripts/ci/compile.mjs preview-cs-preview-002 dev", temp, {
    SSOT_DIR: previewSsot,
    OUT_DIR: previewManifests
  });

  const after = readFileSync(activePath, "utf-8");
  if (before !== after) throw new Error("Active release unstable during preview");
  console.log("Active release stability PASS");
} finally {
