...
async function run() {
  ...
  try {
    ...
    const pageId = `client-entitlement-${Date.now()}`;
    const page_definition = {
      id: pageId,
      surface: "client",
      key: pageId,
      slug: pageId,
      title_key: "client.entitlement",
      module_id: "client",
    ...
    const route_spec = {
      ...
      entitlement_gate_id: "entitlement:blocked",
      priority: 10,
      canonical: true,
      aliases: [],
      deprecation_date: "",
    ...
    if (!failed) throw new Error("Entitlement gate did not block");
    console.log("Entitlement block PASS");
  } finally {
    server.kill();
    temp.cleanup();
  }
