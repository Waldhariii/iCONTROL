...
writeFileSync(
  activationsPath,
  JSON.stringify([
    { tenant_id: "tenant:test", module_id: "module:jobs", state: "active" }
  ], null, 2) + "\n"
);

const releaseId = "mod-jobs-001";
...
if (!routes.includes("route:jobs-list")) throw new Error("Jobs routes missing");
if (routes.includes("route:docs-list") || routes.includes("route:billing-invoices")) {
  throw new Error("Inactive module routes present");
}

console.log("Module activation jobs PASS");
