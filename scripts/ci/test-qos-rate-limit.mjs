...
async function run() {
  ...
  pv[0].rate_limits.rps = 2;
  pv[0].rate_limits.burst = 2;
  ...
  try {
    const reqs = Array.from({ length: 5 }, () =>
      fetch(`${api}/releases`, { headers: { "x-user-id": "user:admin", "x-tenant-id": "tenant:default", "x-scope": "platform:*" } })
    );
    const res = await Promise.all(reqs);
    const limited = res.filter((r) => r.status === 429).length;
    if (limited === 0) throw new Error("Expected rate limiting");
    console.log("QoS rate limit PASS");
  } finally {
    server.kill();
    temp.cleanup();
  }
