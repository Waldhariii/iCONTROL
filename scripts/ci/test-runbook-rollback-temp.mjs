...
try {
  ...
  execSync("node scripts/maintenance/run-runbook.mjs --runbook rb-rollback-last", {
    stdio: "inherit",
    env: { ...process.env, S2S_CI_HMAC: "dummy" }
  });
  const reports = readdirSync(getReportsDir()).filter((f) => f.startsWith("RUNBOOK_rb-rollback-last"));
  if (!reports.length) throw new Error("Runbook rollback report missing");
  console.log("Runbook rollback PASS");
} finally {
  server.kill();
  temp.cleanup();
}
