...
export function applyControlPlaneBootGuards(w: AnyWin): void {
  ...
  try {
    ...
    if (!rt.__FF_GOV_AUDITED__) {
      ...
      rt.__ffGovernanceAudit = govAudit.map((e: any) => ({
        ...e,
        ts,
    ...
    try {
      (w as any).__FF_GOV_AUDIT_FAILED__ = true;
    } catch {}
  }
  ...
  const ff0 = buildFeatureFlagsBootOutcome(undefined, {
    tenant: String(w?.__tenant || "default"),
  });
  const merged = forceOffMany(ff0.flags, forced);
  const ff = buildFeatureFlagsBootOutcome(merged, {
    tenant: String(w?.__tenant || "default"),
  });
  ...
  w.__featureFlags = {
    flags: w.__FEATURE_FLAGS__,
    decisions: w.__FEATURE_DECISIONS__,
  };
  ...
  try {
    ...
    try {
      (w as any).__CP_FORCED_AUDIT_FAILED__ = true;
    } catch {}
  }
}
