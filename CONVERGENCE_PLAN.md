...
- Target interface: `docs/governance/WRITE_GATEWAY_CONTRACT.md`
...
- Source candidates: `app/src/core/governance/rbac/policy.ts`, `config/permissions/rbac.json`, `app/src/runtime/rbac.ts`, `app/src/core/control-plane/guards/pageAccessGuard.ts`
- Strategy: create policy service that reads config/ssot and exposes `authorize` + `guardRouteAccess` consistently.
...
- Source candidates: `config/ssot/design.tokens.json`, `app/src/styles/STYLE_ADMIN_FINAL.css`, `app/src/core/ui/themeManager.ts`, `app/src/core/ui/registry.ts`
...
- SSOT files already in use: `config/ssot/ROUTE_CATALOG.json`, `config/ssot/TENANT_FEATURE_MATRIX.json`, `config/ssot/CAPABILITY_STATUS.json` (only update via governed process).
