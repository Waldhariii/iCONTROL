...
describe("Governance: Reason codes enforcement freeze (v1)", () => {
  ...
  it("rejects any new reason codes in enforcement surface area unless registry updated", () => {
    ...
    const targets = [
      path.join(root, "app/src/core/ports"),
      path.join(root, "app/src/core/write-gateway"),
      path.join(root, "app/src/__tests__"),
      path.join(root, "core-kernel/src/contracts"),
      path.join(root, "core-kernel/src/policy"),
      path.join(root, "core-kernel/domain/errors"),
    ];
