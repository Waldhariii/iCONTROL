...
describe("Governance: navigation is centralized", () => {
  it("no direct location.hash writes in critical modules (use navigate())", () => {
    ...
    // Single exception: app/src/runtime/navigate.ts is the write-gateway.
    const files = [
      "app/src/runtime/navigate.ts",
      "app/src/router.ts",
    ...
    const cleaned = offenders.filter((p) => !p.endsWith("app/src/runtime/navigate.ts"));
