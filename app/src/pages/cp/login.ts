/**
 * ICONTROL_CP_LOGIN_V1
 * Page de connexion pour l'application ADMINISTRATION (/cp)
 */
...
export function renderLoginPage(root: HTMLElement): void {
  debugLog("🟣 renderLogin CP appelé", { id: root.id, className: root.className });
  ...
  if (!document.querySelector('style[data-core-styles]')) {
    ...
    debugLog("✅ Styles de base injectés dans head (CP)");
  }
    ...
    <div style="position:relative;width:100%;height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;background:#1e1e1e;">
      ...
      <div style="${CARD_STYLE}">
        ...
        <div style="margin-top:28px;display:flex;flex-direction:column;gap:12px">
          <input id="u" placeholder="Nom d'utilisateur administrateur" style="${INPUT_STYLE}" />
  ...
  root.innerHTML = html;
  debugLog("✅ HTML injecté dans root CP", { length: root.innerHTML.length });
  ...
  debugLog("✅ Éléments DOM trouvés CP");

  // ICONTROL_LOGIN_SHARED_CODE_V1: Utiliser la fonction partagée pour éviter la duplication
  const submit = createLoginSubmit({ u, p, err, btn }, authenticateManagement, "CP");
