...
// ---- Option B proof: Runtime Config card (ADMIN -> CLIENT) ----
function __runtimeConfigProofCard(): HTMLElement {
  const card = document.createElement("div");
  card.style.cssText = "border:1px solid var(--ic-border,#2b3136);border-radius:12px;padding:12px;min-width:0;box-sizing:border-box;";
  const title = document.createElement("div");
  title.textContent = "Runtime Config";
  title.style.cssText = "font-weight:700;margin-bottom:6px;";
  const body = document.createElement("div");
  body.style.cssText = "opacity:0.9;font-size:13px;line-height:1.35;";
  body.textContent = "ADMIN pilote CLIENT via /api/runtime/config. Valider côté APP: window.__runtimeConfig.";
